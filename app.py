...
with tab_forecast:
    ...
    if st.session_state["forecast_start_date"] < minimum_forecast_start:
        st.session_state["forecast_start_date"] = minimum_forecast_start

    if st.session_state["forecast_end_date"] < st.session_state["forecast_start_date"]:
        st.session_state["forecast_end_date"] = (
            st.session_state["forecast_start_date"] + timedelta(days=29)
        )
    ...
    if st.session_state["forecast_end_date"] < forecast_start_date:
        st.session_state["forecast_end_date"] = (
            forecast_start_date + timedelta(days=29)
        )
    ...
    elif "future_forecast_table" in st.session_state:
        st.info(
...
with tab_help:
    ...
    show_help_card(
        ...
        "онлайн-платформи з на основі моделі машинного навчання.",
        "blue"
    )

    show_help_card(
        "Логіка роботи",
        "У застосунку використовується "
        "готова модель, яка була обрана після порівняння моделей за метриками точності.",
        "purple"
    )
